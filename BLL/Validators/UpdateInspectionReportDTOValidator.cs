...
{
    public class UpdateInspectionReportDTOValidator : AbstractValidator<UpdateInspectionReportDTO>
    {
        public UpdateInspectionReportDTOValidator()
        {
            RuleFor(x => x.Id)
            ...
            RuleFor(x => x.Severity)
                ...
                .Matches("(?i)^(Minor|Medium|High)$").WithMessage("Допустимые значения: Minor, Medium или High.");
            ...
            RuleFor(x => x.FinalCharge)
                .GreaterThanOrEqualTo(0).WithMessage("Итоговая сумма не может быть отрицательной.");

            RuleFor(x => x.Notes)
                .MaximumLength(1000).WithMessage("Примечание не должно превышать 1000 символов.");

            RuleForEach(x => x.Photos)
                .Must(p => !string.IsNullOrWhiteSpace(p)).WithMessage("Путь к фото не может быть пустым.")
